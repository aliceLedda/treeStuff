...
MakeTreeBoth<-function(timesExp,timesNon,alphaExp,Ts,Texp){
timesExp<-as.numeric(timesExp)
timesNon<-as.numeric(timesNon)
PopR<-length(timesExp)/length(timesNon)
#alpha<-2*alphaExp/1000
alpha<-2*alphaExp*PopR
...
while((evolvedExp$height<Ts)&(control==0)){
	while((evolvedExp$height<Texp)&(control==0)){
		if((length(evolvedExp$lineages[,1])>1)|(length(evolvedExp$times)>0)){
		 evolvedExp<-evolutionExpBoth(lineagesExp=evolvedExp$lineages,
...
#treeExp<-makephylo(evoExp);axisPhylo()
