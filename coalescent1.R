...
######preparing evolution
evolved$lineages<-as.matrix(lineag)
evolved$times<-times
evolved$height<-0
evolved$tips<-tips
evolved$evo<-evo
...
######## evolving!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  while((length(evolved$lineages[,1])>1)|(length(evolved$times)>1)){
	if(length(evolved$times)>1){
		evolved<-evolution(lineages=evolved$lineages,tips=evolved$tips,times=evolved$times,treeHeight=evolved$height)
	}else{
		evolved<-evolution(lineages=evolved$lineages,tips=evolved$tips,times=1000000,treeHeight=evolved$height)
	}
...
evo[evo[,1]==max(evo[,1]),1]<-max(tips)+1

tree<-makephylo(evo)

#hei<-c(hei, evolved$height)}
...
extrTime<-function(l){
	...
	}else{time<-rexp(1,rate=0.01)}
	return(time)
}


#### function Extract Leaves Time ---------------------------------------------------
#extracts "times" for the "sampling" of hanging leaves 
#does not pretend to be any rapresentative of the real sampling process, just used for program testing
#parameters: number of total lineages
extrLeaves1<-function(Ntot){
	leavesTime<-c()
	leavesTime[1]<-extrTime(Ntot)
	for (i in 2:(Ntot-2)){
		t<-extrTime(Ntot)
		newTime<-leavesTime[(i-1)]+t
		leavesTime<-c(leavesTime,newTime)
	}
	return(leavesTime)
}
extrLeaves2<-function(Ntot){
	leavesTime<-c()
	leavesTime[1]<-extrTime(2)
	for (i in 3:(Ntot)){
		t<-extrTime(i)
		newTime<-leavesTime[(i-2)]+t
		leavesTime<-c(leavesTime,newTime)
	}
	return(leavesTime)
}
extrLeaves3<-function(Ntot){
	leavesTime<-c()
	leavesTime[1]<-extrTime(2)
	for (i in 3:(Ntot)){
		t<-extrTime(i)
		...
		if(r%/%2==0){
		newTime<-leavesTime[(i-2)]+t
		}else{newTime<-leavesTime[(i-2)]+t+1}
		leavesTime<-c(leavesTime,newTime)
	}
	return(leavesTime)
}
