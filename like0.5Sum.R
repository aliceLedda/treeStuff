...
LogLikeliHood<-function (input, theta){
	...
	for (j in 1:(length(table[,1])-1)){
		i<-length(table[,1])-j
		ToBS<-ToSum(table,i,theta)
		Likeli<-c(Likeli,ToBS)
		}
	Ninternal<-length(which(table[,4]!="l"))
	Likelihood<-log(1/(theta^(Ninternal)))+sum(Likeli)
	return(Likelihood)
}
...
LogLikeliHoodExp<-function (input, theta, alpha){
	if (class(input)=="phylo"){
	table<-PrepareTable(input)
	}else{
	table<-input}
	...
	for (j in 1:(length(table[,1])-1)){
		#j<-j+1
		i<-length(table[,1])-j
		ToS<-ToSumExp(table, i, theta, alpha)
		Likeli<-c(Likeli,ToS)
		}
	Ninternal<-length(which(table[,4]!="l"))
	Likelihood<-log(1/(theta^(Ninternal)))+sum(Likeli)
...
ToSumExp<-function(table,i,theta, alpha){
	ki<-as.numeric(table[(i+1),3])
	ti<-as.numeric(table[i,2])
	timinus1<-as.numeric(table[(i+1),2])
	eti<-exp(alpha*ti)
...
LikelihoodBothOLD<-function(tree, branch, alpha,theta){
