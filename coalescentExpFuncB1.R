...
evolutionExp<-function(lineagesExp, tipsExp, timesExp,treeHeightExp,alphaExp,ratio){
	if(missing(ratio)){ratio<-0}
	#lineagesExp<-as.matrix(lineagesExp)
	Nl<-length(lineagesExp[,1])
	coalTimeExp<-extrTimeExp(l=Nl,treeHeightExp=treeHeightExp,alphaExp=alphaExp, ratio=ratio)  
	coalTimeExp
...
extrTimeExp<-function(l, treeHeightExp, alphaExp,ratio){
	if ((missing(ratio))|(ratio==0)) {ratio=1}
	if(l>1){
		par<-l*(l-1)/(2*ratio)
		timeO<-rexp(1,rate=par)
		if(alphaExp==0){timeNew<-timeO
		}else{
...
MakeExpTree<-function(alphaExp,timesExp,timesNon){
	if(missing(timesNon)){ratio<-0
		}else{ratio<-length(timesExp)/length(timesNon)}
...
evolvedExp<-list(lineages=as.matrix(lineag),times=timesExp,height=0,tips=tipsExp,evo=evo,ratio=ratio)
...
return(list(evo=evo, height=evolvedExp$height,evolvedExp=evolvedExp))
}
