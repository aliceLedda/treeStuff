...
#parameters: vector of extant leaves, vector of leaves times0

evolution<-function(lineages, tips, times0,treeHeight){
	#lineages<-as.matrix(lineages)
	Nl<-length(lineages[,1])
	coalTime<-extrTime(Nl,treeHeight)  
	coalTime
	#while(((treeHeight+coalTime)<times0[1])&(length(lineages[,1])==1)){
	#	coalTime<-extrTime(Nl,treeHeight) 
	#}
	
	if(((treeHeight+coalTime)<times0[1])|(identical(evolved$times0,numeric(0)))){
		coal<-chooseLeaves(length(lineages[,1]))
		ma<-matrix(nrow=2,ncol=3,data=0)
		ma[,2]<-c(lineages[coal[1],1],lineages[coal[2],1])
		if(max(lineages[,1])<(length(times0)+(max(tips))+1)){
			ma[,1]<-rep((length(times0)+(max(tips))+1),2)
			lineages<-rbind(lineages,c(0,0))
			lineages[length(lineages[,1]),1]<-(length(times0)+(max(tips))+1)
		...
		return(list(lineages=lineages,tips=tips,evo=ma,height=treeHeight,times0=times0))
	}else{
		lineages<-rbind(lineages,c(0,0))
		lineages[length(lineages[,1]),1]<-(max(tips)+1)
		lineages[length(lineages[,1]),2]<-times0[1]
		tips<-c(tips,(max(tips)+1))
		treeHeight<-times0[1]
		times0<-times0[-1]
		return(list(lineages=lineages,tips=tips,evo=0,height=treeHeight,times0=times0))
	}
...
extrTime<-function(l, treeHeight){
	if(l>1){
		par<-l*(l-1)/2
...
lineag[1,]<-c(1,times0[1]) #it is initialised with one lineage at time 0
times0<-times0[-1]
...
######preparing evolution
evolved<-list(lineages=as.matrix(lineag),times0=times0,height=0,tips=tips,evo=evo)


######## evolving!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	#while((length(evolved$lineages[,1])+length(evolved$times0)>1){	
  while((length(evolved$lineages[,1])>1)|(length(evolved$times0)>0)){
		
	evolved<-evolution(lineages=evolved$lineages,tips=evolved$tips,times0=evolved$times0,treeHeight=evolved$height)
...
lineages<-evolved$lineages

treeHeight<-evolved$height
tips<-evolved$tips
evo<-evolved$evo

times0<-evolved$times0


evolved<-list(lineages=lineages,times0=times0,height=treeHeight,tips=tips,evo=evo)
