...
evolutionNon<-function(lineagesNon, tipsNon, timesNon,heightNon){
	...
	coalTime
	if(((heightNon+coalTime)<timesNon[1])|(identical(timesNon,numeric(0)))){
...
#parameters:the vector of times at which leaves have to appear
...
######preparing evolution
evolvedNon<-list(lineages=as.matrix(lineag),times=timesNon,height=0,tips=tipsNon,evo=evo)
