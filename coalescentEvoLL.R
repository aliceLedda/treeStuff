...
extrTime<-function(l, treeHeight, alpha,ratio,Texp,expns){
	...
	if(l>1){
		...
		if((alpha==0)|(expns=="none")){timeNew<-timeO
			...
			if(expns=="exp"){
				tauE<-function(ARGU){
						tauE<-(1/(ratio*alpha))*(exp(alpha*ARGU)-1)
						return(tauE)
					}
					tprime<-function(ARGU){
						tprime<-(1/alpha)*log((alpha*ARGU)+1)
						return(tprime)
					}
			}
			else{
				if (expns=="lin"){
					tauE<-function(ARGU){
						tauE<-(-Texp/ratio)*log(alpha*(Texp-ARGU))
						return(tauE)
					}
					tprime<-function(ARGU){
						tprime<-Texp-(exp(ARGU/Texp)/alpha)
						return(tprime)
					}
				}
				else{
					if(expns=="Xav"){
						At2<-alpha*Texp/2
						tauE<-function(ARGU){
							tauE<-((-2)/(alpha*ratio))*tanh(At2)*log(sinh(alpha*(Texp-ARGU)/2)/sinh(At2))
							return(tauE)
						}
						tprime<-function(ARGU){
							tprime<-Texp-((2/alpha)*asinh(sinh(At2)*exp(-alpha*ARGU/(2*tanh(At2)))))
							return(tprime)					
						}
					}
				}
			...
			part<-ratio*(timeO+HeightMod)  #### Modified tree height + new time
			timeNew<-tprime(part)-tprime(HeightMod) ### New time= (tree Height+ new time)-tree Height
	}
	}else{timeNew<-100000}
