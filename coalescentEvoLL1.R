...
#source("coalescentBothEvo4.R")

####  function Evolution -------------------------------------------------------
#starts the evolution matrix
#parameters: vector of extant leaves, vector of leaves timesExp

evolution<-function(lineages, tips, times,treeHeight,alpha,ratio, Texp, expns){
	if(missing(ratio)){ratio<-0}
	#lineagesExp<-as.matrix(lineagesExp)
	Nl<-length(lineages[,1])

	coalTime<-extrTime(l=Nl,treeHeight=treeHeight,alpha=alpha, ratio=ratio, Texp=Texp,expns=expns)  
	...
	if(((treeHeight+coalTime)<times[1])|(identical(times,numeric(0)))){
		...
		return(list(lineages=lineages,tips=tips,evo=ma,treeHeight=treeHeight,times=times))
	}else{
		lineages<-rbind(lineages,c(0,0))
		...
		treeHeight<-times[1]
		times<-times[-1]
		return(list(lineages=lineages,tips=tips,evo=0,treeHeight=treeHeight,times=times))
	}
...
extrTime<-function(l, treeHeight, alpha,ratio,Texp,expns){
	...
	if(l>1){
		...
		if((alpha==0)|(expns=="none")){timeNew<-timeO
		}else{  #### I define for every expns two functions that I will need to then compute the time in the non expanding frame

			if(expns=="exp"){
				tauE<-function(ARGU){
						tE<-(1/(ratio*alpha))*(exp(alpha*ARGU)-1)
						return(tE)
					}
					tprime<-function(ARGU){
						tp<-(1/alpha)*log((alpha*ARGU)+1)
						return(tp)
					}
			}
			else{
				if (expns=="lin"){
					tauE<-function(ARGU){
						tE<-(-Texp/ratio)*log(alpha*(Texp-ARGU))
						return(tE)
					}
					tprime<-function(ARGU){
						tp<-Texp-(exp(ARGU/Texp)/alpha)
						return(tp)
					}
				}
				else{
					...
					# 	At2<-alpha*Texp/2
					# 	tauE<-function(ARGU){
					# 		#tE<-((-2)/(alpha*ratio))*tanh(At2)*log(sinh(alpha*(Texp-ARGU)/2)/sinh(At2))
					# 		tE<-((-2)/(alpha*ratio))*tanh(At2)*((alpha*(Texp-ARGU)/2)+log(1-exp(-alpha*(Texp-ARGU)))-At2-log(1-exp(-2*At2)))
					# 		return(tE)
					# 	}
					# 	tprime<-function(ARGU){
					# 		tp<-Texp-((2/alpha)*asinh(sinh(At2)*exp(-alpha*ARGU/(2*tanh(At2)))))
					# 		return(tp)					
					# 	}
					# }
					 if(expns=="xav"){
					 ...
					 #	At2<-alpha*Texp/2
						tauE<-function(ARGU){
					# 		t#tE<-((-2)/(alpha*ratio))*tanh(At2)*log(sinh(alpha*(Texp-ARGU)/2)/sinh(At2))
					# 		tE<-((-2)/(alpha*ratio))*tanh(At2)*((alpha*(Texp-ARGU)/2)+log(1-exp(-alpha*(Texp-ARGU)))-At2-log(1-exp(-2*At2)))
					 		if(ARGU[1]<=(Texp-1/alpha)){tE<-ARGU} else {tE<-Texp-1/alpha-log(alpha*(Texp-ARGU))/alpha}
					 		tE<-tE/ratio
					 		return(tE)
					 	}
					 	tprime<-function(ARGU){
					# 		tp<-Texp-((2/alpha)*asinh(sinh(At2)*exp(-alpha*ARGU/(2*tanh(At2)))))
							if(ARGU[1]<=Texp-1/alpha){tp<-ARGU} else {tp<-Texp-exp(-alpha*(ARGU-Texp+1/alpha))/alpha}
					 		return(tp)					
					 	}
					 }
				}
			...
			#print(treeHeight)
			HeightMod<-tauE(treeHeight)   ####Modified tree Height
			part<-(timeO+HeightMod)  #### Modified tree height + new time
			timeNew<-tprime(ratio*part)-tprime(ratio*HeightMod) ### New time= (tree Height+ new time)-tree Height
	}
	}else{timeNew<-100000}
...
evolved<-list(lineages=as.matrix(lineag),
			times=times,
			treeHeight=0,
			tips=tips,
			evo=evo,
...
return(list(evo=evo, height=evolved$treeHeight,evolved=evolved))
}
...
if(((alpha==1)&(expns!="none"))|((alpha!=1)&(expns=="none"))) {stop("your request is incoherent")}
...
# alphaExp<-20
# tree<-MakeTree(times=rep(0,20),alpha=1,ratio=1,expns="none")
# plot.phylo(tree)
